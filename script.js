...
function bindReviews() {
  ...
  function updateReviewHeight() {
    if (!mobileQuery.matches) {
      trackElement.style.removeProperty("height");
      return;
    }
    const padding = parseFloat(getComputedStyle(trackElement).paddingBottom) || 0;
    trackElement.style.height = `${Math.ceil(cards[activePage].getBoundingClientRect().height + padding)}px`;
  }
  ...
  if ("ResizeObserver" in window) {
    let trackWidth = trackElement.clientWidth;
    new ResizeObserver(() => {
      if (trackElement.clientWidth === trackWidth) return;
      trackWidth = trackElement.clientWidth;
      goToPage(activePage, true);
      updateReviewHeight();
    }).observe(trackElement);
    const cardResizeObserver = new ResizeObserver(updateReviewHeight);
