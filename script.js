...
/* =========================================================
   AVALIAÇÕES: GRUPOS DE TRÊS, COM TOQUE E TECLADO
========================================================= */
function bindReviews() {
  const trackElement = $("#reviewsTrack");
  const pages = $$(".reviews-page", trackElement);
  const previous = $("#reviewsPrev");
  const next = $("#reviewsNext");
  const status = $("#reviewsStatus");
  const total = $$(".review-card", trackElement).length;
  let activePage = 0;
  ...
  function updateControls() {
    ...
    status.textContent = `${activePage * 3 + 1} a ${Math.min(activePage * 3 + 3, total)} de ${total}`;
  }
